...
pub fn poly_invntt_scalar(poly: &mut Poly) {
    let mut k = 0;
    let mut len = 2;
    while len <= 128 {
        let mut start = 0;
    ...
    for i in 0..256 {
        let t = i32::from(poly.coeffs[i]) * i32::from(INV_N);
        poly.coeffs[i] = barrett_reduce_scalar(montgomery_reduce_scalar(t));
    }
}
...
pub unsafe fn poly_invntt_avx(poly: &mut Poly) {
    ...
    // Fully Vectorized iNTT Loops (100% AVX2 Native Architecture)
    while len <= 128 {
        let mut start = 0;
        while start < 256 {
            // Unrolling and adapting vectors dynamically for standard layouts
            let zeta_idx = (k + 1) & 127;
            let zeta = _mm256_set1_epi16(INV_ZETAS[zeta_idx]);
            k += 1;

            // Strict vector step size processing
            let step = if len < 16 { 16 } else { len };

            for j in (start..(start + len)).step_by(step) {
            ...
            // Ensuring branch coordinates move perfectly according to layer depth
            start += if len < 16 { 32 } else { 2 * len };
        }
        len <<= 1;
    }

    // Parallel Array Scale Factor Vector Inversion Block
...
pub fn poly_invntt_dispatch(poly: &mut Poly) {
    ...
    poly_invntt_scalar(poly);
}

fn main() {
    println!("🚀 RUNNING FULLY VECTORIZED ULTRAPASS AVX2 iNTT ENGINE...");
    ...
    for _ in 0..10000 {
        ...
        poly_invntt_dispatch(&mut active_poly);
    }

    let duration = start.elapsed();
    println!("⚡ BENCHMARK COMPLETE: 10,000 invNTT Executions took: {:?}", duration);
    println!("📊 Sample Coefficients Output (First 5): {:?}", &active_poly.coeffs[0..5]);
          }
